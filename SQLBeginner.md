...
When defining or calling text values, use single quotes around the value (ex.```'string'```). Some databases do allow double quotation marks instead, but in MS SQL Server, use single quotes. Numeric fields does not use any quotation marks.
